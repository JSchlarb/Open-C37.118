...
unsigned short CONFIG_Frame::pack(unsigned char **buff)
{
	...
	*lptr = htonl(this->FRACSEC_get());
	aux_buff += 4;
	lptr = (unsigned long *)(aux_buff);
	*lptr = htonl(this->TIME_BASE_get());
	aux_buff += 4;
	shptr = (unsigned short *)(aux_buff);
	*shptr = htons(this->NUM_PMU_get());
	aux_buff += 2;
	...
	for (int i = 0; i < this->NUM_PMU_get(); i++)
	{
		...
		// Get another fields
		shptr = (unsigned short *)(aux_buff);
		*shptr = htons(this->pmu_station_list[i]->IDCODE_get());
		aux_buff += 2;
		shptr = (unsigned short *)(aux_buff);
		*shptr = htons(this->pmu_station_list[i]->FORMAT_get());
		aux_buff += 2;
		shptr = (unsigned short *)(aux_buff);
		*shptr = htons(this->pmu_station_list[i]->PHNMR_get());
		aux_buff += 2;
		shptr = (unsigned short *)(aux_buff);
		*shptr = htons(this->pmu_station_list[i]->ANNMR_get());
		aux_buff += 2;
		shptr = (unsigned short *)(aux_buff);
		*shptr = htons(this->pmu_station_list[i]->DGNMR_get());
		aux_buff += 2;
		...
		for (int j = 0; j < this->pmu_station_list[i]->PHNMR_get(); j++)
		{
			...
			*lptr = htonl(this->pmu_station_list[i]->PHUNIT_get(j));
			aux_buff += 4;
		}
		// Analog
		for (int j = 0; j < this->pmu_station_list[i]->ANNMR_get(); j++)
		{
			lptr = (unsigned long *)(aux_buff);
			*lptr = htonl(this->pmu_station_list[i]->ANUNIT_get(j));
			aux_buff += 4;
		}
		// Digital
		...
		{
			lptr = (unsigned long *)(aux_buff);
			*lptr = htonl(this->pmu_station_list[i]->DGUNIT_get(j));
			aux_buff += 4;
		}

		// FNOM
		shptr = (unsigned short *)(aux_buff);
		*shptr = htons(this->pmu_station_list[i]->FNOM_get());
		aux_buff += 2;

		// CFGCNT
		shptr = (unsigned short *)(aux_buff);
		*shptr = htons(this->pmu_station_list[i]->CFGCNT_get());
		aux_buff += 2;
	...
	// DATA RATE
	shptr = (unsigned short *)(aux_buff);
	*shptr = htons(this->DATA_RATE_get());
	aux_buff += 2;
